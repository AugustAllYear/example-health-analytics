...
for _ in range(num_claims):
	...
	"provider_id": f"P{random.randint(1, config["generations']['num_providers']):03d}",
...
claims_path = os.path.join(data_dir, "claimes.parquet")
df_claims.to_parquet(claims_path, index=False)

# -- generate providers (nested JSON) ---
num_providers = config("config["generation"]["num_providers"]
providers = []
...
print(f"Generated {num_claimes} claime -> {claims_path}")
